...
async function main() {
  ...
  for (let i = 0; i < userInput.addresses.length; i++) {
    ...
    if (
      ...
      exportVariable(
        JSON.stringify(obj),
        userInput.addresses[i].name + " " + obj.address + ".json"
      );
  ...
  console.log(
    "The following table lists all found rewards and values are expressed in " +
      obj.currency
  );

  const DOT = {
    Name: "DOT",
    "Nr. Payouts": numberPayouts.DOT,
    "Number of Tokens": totalStaked.DOT,
    Value: totalFiatToken.DOT,
  };
  const KSM = {
    Name: "KSM",
    "Nr. Payouts": numberPayouts.KSM,
    "Number of Tokens": totalStaked.KSM,
    Value: totalFiatToken.KSM,
  };
  const GLMR = {
    Name: "GLMR",
    "Nr. Payouts": numberPayouts.GLMR,
    "Number of Tokens": totalStaked.GLMR,
    Value: totalFiatToken.GLMR,
  };
  const MOVR = {
    Name: "MOVR",
    "Nr. Payouts": numberPayouts.MOVR,
    "Number of Tokens": totalStaked.MOVR,
    Value: totalFiatToken.MOVR,
  };
  const SDN = {
    Name: "SDN",
    "Nr. Payouts": numberPayouts.SDN,
    "Number of Tokens": totalStaked.SDN,
    Value: totalFiatToken.SDN,
  };
  const ASTR = {
    Name: "ASTR",
    "Nr. Payouts": numberPayouts.ASTR,
    "Number of Tokens": totalStaked.ASTR,
    Value: totalFiatToken.ASTR,
  };

  console.table([DOT, KSM, GLMR, MOVR, ASTR, SDN]);
  console.log(
    "The total value of all payouts is " +
      totalFiat +
      " " +
      obj.currency +
  ...
  console.log(
    "For more information, open the CSV file(s) or copy the content of the JSON file(s) into http://jsonviewer.stack.hu/ (click format)."
  );
}
main()
  .catch(console.error)
