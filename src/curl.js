  ...
  do {
    ...
    for (let i = 0; i < obj.data.numberOfDays; i++) {
      for (let x = 0; x < loopIndex; x++) {
        let tmp = dateToString(
          new Date(stakingObject.data.list[x].block_timestamp * 1000)
        );
        if (
          (tmp == obj.data.list[i].day) &
          (stakingObject.data.list[x].event_id == "Reward")
        ) {
          found += 1;
          ...
          if (obj.data.list[i].numberPayouts >= 1) {
            obj.data.list[i].amountPlanks =
              obj.data.list[i].amountPlanks +
              parseInt(stakingObject.data.list[x].amount);
            obj.data.list[i].numberPayouts = obj.data.list[i].numberPayouts + 1;
            obj.data.list[i].blockNumber =
              obj.data.list[i].blockNumber +
              " and " +
              stakingObject.data.list[x].block_num;
            obj.data.list[i].extrinsicHash =
              obj.data.list[i].extrinsicHash +
              " and " +
              stakingObject.data.list[x].extrinsic_hash;
            // if an entry has only the default values we add the ones from the staking object.
          } else {
            obj.data.list[i].amountPlanks = parseInt(
              stakingObject.data.list[x].amount
            );
            obj.data.list[i].numberPayouts = obj.data.list[i].numberPayouts + 1;
...
async function getStakingObject(address, page, network) {
  ...
  let url =
    "https://" + network + ".api.subscan.io/api/scan/account/reward_slash";
