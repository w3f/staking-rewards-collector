...
export function calculateMetrics(obj) {
  ...
  for (let i = 0; i < obj.data.numberOfDays; i++) {
    ...
    obj.data.list[i].amountHumanReadable =
      obj.data.list[i].amountPlanks * normalization;
    obj.data.list[i].valueFiat =
      obj.data.list[i].amountHumanReadable * obj.data.list[i].price;
    ...
    obj.totalAmountHumanReadable =
      obj.totalAmountHumanReadable + obj.data.list[i].amountHumanReadable;
  }

  obj.totalValueFiat = round(obj.totalValueFiat, 2);
  obj.currentValueRewardsFiat = round(
    obj.totalAmountHumanReadable * obj.data.list[0].price,
    2
  );
...
function _calculateAnnualizedReturn(obj) {
  ...
  daysBetweenRewards =
    (transformDDMMYYYtoUnix(obj.lastReward) -
      transformDDMMYYYtoUnix(obj.firstReward)) /
      60 /
      60 /
      24 +
    1;
...
export function checkPriceAvailablilty(userInput, network) {
  ...
  if (
    (end.valueOf() < 1597708800000) &
    (network == "polkadot") &
    (priceData == "true")
  ) {
    console.log(
      "Your requested time window lies before prices are available for " +
        network.toUpperCase() +
        ". Switching off price data."
    );
    priceData = "false";
  }

  if (
    (end.valueOf() < 1568851200000) &
    (network == "kusama") &
    (priceData == "true")
  ) {
    console.log(
