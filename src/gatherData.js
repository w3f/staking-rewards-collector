...
export async function gatherData(start, end, network, address, currency, priceData, startBalance, ticker){
   
    let obj = {};
    let daysArray = [];

    daysArray = makeDaysArray(new Date(start), new Date(end));
    obj = initializeObject(daysArray, network, address, currency, startBalance, ticker);
    if(priceData == 'true'){
